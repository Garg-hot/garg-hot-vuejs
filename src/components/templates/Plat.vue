...
const updatePlat = async () => {
  ...
  try {
    ...
    await platService.updatePlat(editPlat.value.id, platToUpdate);
  ...
  <div>
    ...
    <ul>
      <li v-for="plat in plats" :key="plat.id">
        <strong>{{ plat.nom }}</strong> - {{ plat.description }} ({{ plat.duration }} min)
        <button @click="() => {
          editPlat = {
            ...plat,
            categorie: plat.categorie.id // On s'assure de bien récupérer l'ID de la catégorie
          };
        }">Éditer</button>
